...
def test_embedding_profile_follows_runtime_profile_when_no_override() -> None:
    ...
    assert settings.embedding_profile() == ("http://qwen:8000/v1", "k-qwen", "qwen-embed")
...
def test_embedding_profile_can_use_dedicated_provider_override() -> None:
    settings = Settings(
        ...
        embedding_api_key="k-embed",
        embedding_api_model="embed-model",
    )
    assert settings.embedding_profile() == ("https://api.siliconflow.cn/v1", "k-embed", "embed-model")
